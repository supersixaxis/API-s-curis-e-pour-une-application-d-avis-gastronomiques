...
const fs = require('fs');
// nos actions pour le dossier routes/stuff
...
exports.deleteSauce = (req, res, next) => {
 ...
 exports.getOneSauce = (req, res, next) => {
    Sauce.findOne({ _id: req.params.id }) // findOne permet de trouver l'objet sur lequel on clique pour l'afficher, ici grace a son id
        .then(sauce => res.status(200).json(sauce))
        .catch(error => res.status(404).json({ error }));
};

exports.getAllSauces = (req, res, next) => {
    Sauce.find() // find() permet de prendre tout les objets d'un coup
